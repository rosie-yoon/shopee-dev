...
# automation_steps의 헬퍼/접근법 재사용 (헤더 정규화, 컬럼 탐색, TEM 이름 등)
from item_uploader.automation_steps import (  # noqa
...
# ------------------------------------------------------------------------------
# 내부: TemplateDict 로드 (TopLevel → 헤더 배열)
# ------------------------------------------------------------------------------
...
def run_step_C1(sh: gspread.Spreadsheet, ref: Optional[gspread.Spreadsheet]):
    ...
    # 헤더는 C2에서 카테고리별로 삽입(복제 Step1 방식과 동일한 행구조)
    with_retry(lambda: tem_ws.update(values=[[""]], range_name="A1"))
    print("C1 Done.")

# ------------------------------------------------------------------------------
# Collection 헤더 인덱스 탐색 (유연한 별칭 지원)
# ------------------------------------------------------------------------------
def _collect_indices(header_row: List[str]) -> Dict[str, int]:
    ...
    return {
        "create": idx("create", ["use", "apply"]),              # A열(없으면 0번 취급)
...
# ------------------------------------------------------------------------------
# C2: Collection → TEM_OUTPUT 매핑 (A=True / forward-fill / 지정 매핑)
# ------------------------------------------------------------------------------
def run_step_C2(sh: gspread.Spreadsheet, ref: gspread.Spreadsheet):
    ...
    # 2) Collection 데이터 읽기(+ forward-fill)
    ...
    # 3) 카테고리 버킷 구성 (복제 Step1과 동일 구조: ["" + headers] + [pid + data...])
    ...
    for r in range(1, len(ff_vals)):
        ...
        # TEM 행 생성
        tem_row = [""] * len(headers)
        # 매핑: K→B, H→C, J→D, B→F, G→"Options", F→H, C→N, D→AE
        ...
        set_if_exists(headers, tem_row, "brand", brand)                       # AE (Step4에서 코드로 치환됨)

        # PID: variation || sku || "ROW{r}"
...
# ------------------------------------------------------------------------------
# C5: 이미지 생성 + Variation Integration 복원 (Step5 후 보정)
# ------------------------------------------------------------------------------
def run_step_C5_images(
    ...
    # 1) Collection에서 SKU→Variation / Variation→DetailCount 맵 구성 (forward-fill 반영)
    coll_ws = safe_worksheet(sh, "Collection")
    coll_vals = with_retry(lambda: coll_ws.get_all_values()) or []
    ...
    for r0, row in enumerate(tem_vals):
        ...
        if (row[1] if len(row) > 1 else "").strip().lower() == "category":
            ...
            idx_varno   = _find_col_index(cur_headers, "variationintegration")  # "No." 포함 케이스도 커버됨
            # Item Image 1~8
            idx_items = []
            for k in range(1, 9):
        ...
        # 2-2) I열 Image per Variation = OptionBase/SKU
        ...
        # 2-3) O열 Cover = CoverBase/(var or sku) + _C_{shop_code}.jpg
